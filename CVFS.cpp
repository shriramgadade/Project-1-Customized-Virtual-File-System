...
void displayHelp()
{
    ...
    printf("Lseek : used to change the fileoffset\n");
...
int main()
{
    ...
    {
        fflush(stdin); // use after scanf statement also clears the input buffer in certain compilers
        strcpy(str,""); // string is empty

        printf("\n Shriram Gadade  CVFS : > ");

        fgets(str,80,stdin);  // use :-- scanf("&[^'\n']s",str);
        ...
        else if(count ==3)
        {
            ...
            {
                ret = CreateFile(command[1],atoi(command[2])); // atoi convert a string into integer numirecal
                if(ret >= 0)
            ...
            else if(strcmp(command[0],"open") == 0)
            {
                ...
                if(ret == -3)
                    printf("ERROR : Permissoion denied\n");
